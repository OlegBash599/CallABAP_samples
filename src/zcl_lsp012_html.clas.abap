*----------------------------------------------------------------------*
*       CLASS ZCL_LSP012_HTML DEFINITION
*----------------------------------------------------------------------*
*
*----------------------------------------------------------------------*
class ZCL_LSP012_HTML definition
...
public section.
  ...
  types:
    BEGIN OF ts_index_str " определяем структуру " определяем структуру
            , index TYPE syindex
            , line TYPE string
        , END OF ts_index_str .
  ...
  class-methods GET_INSTANCE
    ...
    returning
      value(RO_OBJ) type ref to ZCL_LSP012_HTML .
  methods CONSTRUCTOR
    importing
      !IV_TITLE type C .
  ...
  PRIVATE SECTION.
    ...
    CLASS-DATA mo_html TYPE REF TO zcl_lsp012_html .
...
CLASS ZCL_LSP012_HTML IMPLEMENTATION.
  ...
  METHOD show_in_container.
    ...
    cl_abap_browser=>show_html(
      EXPORTING
        ...
        title        =  me->mv_title_win   " Window Title
*        size         = CL_ABAP_BROWSER=>MEDIUM    " Size (S,M.L,XL)
        modal        = abap_false    " Display as Modal Dialog Box
*        html_string  =     " HTML String
*        printing     = ABAP_FALSE
         buttons      = cl_abap_browser=>navigate_html
*        format       = CL_ABAP_BROWSER=>LANDSCAPE    " Landscape/portrait format
